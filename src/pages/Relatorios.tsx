    ...
    <div className="p-8 bg-gray-50 min-h-screen">
      ...
      <div className="flex justify-between items-center mb-6 print:hidden">
        ...
        <div className="flex gap-2">
          <button onClick={exportarCSV} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded font-bold transition shadow-sm">
            Baixar Excel (CSV)
          </button>
          <button onClick={exportarPDF} className="bg-gray-800 hover:bg-gray-900 text-white px-4 py-2 rounded font-bold transition shadow-sm">
            Imprimir / PDF
          </button>
        </div>
      </div>
      ...
      <form onSubmit={lidarComPesquisa} className="bg-white p-6 rounded shadow-sm border mb-8 print:hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
          ...
          <div className="flex flex-col gap-1">
            <label className="text-sm font-semibold text-gray-600">Buscar por Produto/Receita</label>
          ...
          <button type="submit" className="bg-emerald-600 hover:bg-emerald-700 text-white p-2.5 rounded font-bold transition shadow-sm">
            Pesquisar
          </button>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <div className="bg-white p-4 rounded border shadow-sm">
          ...
          <p className="text-2xl font-bold text-gray-800">{totais.qtd} un</p>
        </div>
        <div className="bg-white p-4 rounded border shadow-sm">
          <p className="text-xs font-bold text-gray-500 uppercase">Custo dos Produtos</p>
          <p className="text-2xl font-bold text-red-600">R$ {totais.custo.toFixed(2)}</p>
        </div>
        <div className="bg-white p-4 rounded border shadow-sm">
          <p className="text-xs font-bold text-gray-500 uppercase">Faturamento Bruto</p>
          <p className="text-2xl font-bold text-blue-600">R$ {totais.faturamento.toFixed(2)}</p>
        </div>
        <div className="bg-white p-4 rounded border shadow-sm border-b-4 border-emerald-500">
          ...
          <p className="text-2xl font-bold text-emerald-600">R$ {totais.lucro.toFixed(2)}</p>
        </div>
      </div>
      ...
      <div className="flex gap-4 border-b mb-6 print:hidden">
        <button 
          ...
          className={`pb-2 px-4 font-bold transition-all ${abaAtiva === 'historico' ? 'border-b-2 border-gray-800 text-gray-800' : 'text-gray-400 hover:text-gray-600'}`}
        >
          Histórico Detalhado
        </button>
        <button 
          onClick={() => setAbaAtiva('analise')} 
          className={`pb-2 px-4 font-bold transition-all ${abaAtiva === 'analise' ? 'border-b-2 border-gray-800 text-gray-800' : 'text-gray-400 hover:text-gray-600'}`}
        >
      ...
      <div className={`${abaAtiva === 'historico' ? 'block' : 'hidden'} print:block mb-8`}>
        <div className="bg-white border rounded shadow-sm overflow-hidden">
          <div className="p-4 bg-gray-50 border-b print:bg-white flex justify-between items-center">
            <h3 className="font-bold text-gray-800">
              Listagem de Vendas ({filtroInicio || filtroFim ? 'Período Selecionado' : 'Mês Atual'})
            </h3>
            <span className="text-sm font-bold text-blue-600">{vendasFiltradas.length} registros</span>
          </div>
          <div className="max-h-96 overflow-y-auto print:max-h-none print:overflow-visible">
            {vendasFiltradas.length === 0 ? (
              ...
              <table className="w-full text-left text-sm">
                <thead className="bg-gray-100 text-gray-600 sticky top-0 print:static">
                  <tr>
                ...
                <tbody className="divide-y">
                  {vendasFiltradas.slice().reverse().map((v: any, i: number) => (
                    <tr key={i} className="hover:bg-gray-50">
                      <td className="p-3 text-gray-500">{new Date(v.dataHoraISO).toLocaleString([], { dateStyle: 'short', timeStyle: 'short' })}</td>
      ...
      <div className={`${abaAtiva === 'analise' ? 'block' : 'hidden'} print:block grid grid-cols-1 md:grid-cols-2 gap-6`}>
        ...
        <div className="bg-white border rounded shadow-sm">
          <div className="p-4 bg-gray-50 border-b print:bg-white">
            ...
            <p className="text-xs text-gray-500">Produtos que mais geraram retorno no período selecionado</p>
          </div>
          <div className="p-4">
                ...
                {rankingProdutos.map((item, index) => (
                  <div key={item.nome} className="flex justify-between items-center border-b pb-2">
                    <div className="flex items-center gap-3">
                      <span className={`w-6 h-6 flex items-center justify-center rounded-full text-xs font-bold ${index === 0 ? 'bg-amber-100 text-amber-700' : index === 1 ? 'bg-gray-200 text-gray-700' : index === 2 ? 'bg-orange-100 text-orange-700' : 'bg-gray-100 text-gray-500'}`}>
                        {index + 1}
                      </span>
                      <div>
                    ...
                    <div className="text-right">
                      <p className="font-bold text-emerald-600 text-sm">R$ {Number(item.lucro).toFixed(2)}</p>
        ...
        <div className="bg-white border rounded shadow-sm">
          <div className="p-4 bg-red-50 border-b print:bg-white print:border-b-2 print:border-red-500">
            ...
            <p className="text-xs text-red-600">Produtos cadastrados com ZERO saídas no período selecionado</p>
          </div>
          <div className="p-4">
              ...
              <div className="space-y-3">
                {produtosParados.map((item: any, index: number) => (
                  <div key={index} className="flex justify-between items-center border-b pb-2">
                    ...
                    <div className="text-right">
                      <p className="font-medium text-red-600 text-sm">R$ {Number(item.precoCusto || 0).toFixed(2)}</p>
